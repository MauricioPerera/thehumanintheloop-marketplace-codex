...
def validate_plugin_manifests(plugin_root, relative, category, errors):
    ...
    if codex.get("interface", {}).get("category") != category:
        errors.append(f"Plugin category differs from marketplace entry: {relative}")
        ...
        for entry in entries:
            ...
            if not name or not relative: errors.append("Every plugin entry needs name and source/path")
                ...
                if (plugin_root / ".claude-plugin" / "plugin.json").exists() and (plugin_root / ".codex-plugin" / "plugin.json").exists():
                    validate_plugin_manifests(plugin_root, relative, entry.get("category"), errors)
