#!/usr/bin/env python3
"""Validate a GitHub security advisory draft."""
import argparse,json,re,sys
from pathlib import Path
def validate(text):
    errors=[]
    for label,p in [("vulnerability",r"(?i)\b(?:vulnerabilit(?:y|ies)|vulnerabilidad(?:es)?)\b"),("versions",r"(?i)\b(?:versions?|versi[oó]n(?:es)?)\b"),("severity",r"(?i)\b(?:severity|severidad|CVSS)\b"),("mitigation",r"(?i)\b(?:mitigation|mitigaci[oó]n|fix|correcci[oó]n)\b"),("disclosure",r"(?i)\b(?:disclosure|divulgaci[oó]n|timeline)\b")]:
        if not re.search(p,text): errors.append(f"Missing {label} information.")
    if len(text.split())<70: errors.append("Advisory must contain at least 70 words.")
