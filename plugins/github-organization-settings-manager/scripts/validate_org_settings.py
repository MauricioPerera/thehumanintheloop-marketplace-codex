#!/usr/bin/env python3
"""Validate an organization settings audit."""
import argparse,json,re,sys
from pathlib import Path
def validate(text):
    errors=[]
    for label,p in [("organization",r"(?i)\b(?:organization|organizaci[oó]n)\b"),("permissions",r"(?i)\b(?:permission|permiso|visibility|visibilidad)\b"),("policy",r"(?i)\b(?:policy|pol[ií]tica|forking|actions|security)\b"),("impact",r"(?i)\b(?:impact|impacto|risk|riesgo)\b"),("rollback",r"(?i)\b(?:rollback|revert|revers)\b")]:
        if not re.search(p,text): errors.append(f"Missing {label} information.")
    if len(text.split())<70: errors.append("Organization settings audit must contain at least 70 words.")
