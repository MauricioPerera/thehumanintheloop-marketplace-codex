#!/usr/bin/env python3
"""Validate a GitHub fork governance plan."""
import argparse,json,re,sys
from pathlib import Path
def validate(text):
    errors=[]
    for label,p in [("fork",r"(?i)\b(?:fork|forks)\b"),("owner",r"(?i)\b(?:owner|propietario)\b"),("visibility",r"(?i)\b(?:visibility|visibilidad|public|private)\b"),("sync",r"(?i)\b(?:sync|sincron|diverg)\b"),("rollback",r"(?i)\b(?:rollback|revert|revers)\b")]:
        if not re.search(p,text): errors.append(f"Missing {label} information.")
    if len(text.split())<70: errors.append("Fork plan must contain at least 70 words.")
