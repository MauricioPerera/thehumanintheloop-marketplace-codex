#!/usr/bin/env python3
"""Validate a GitHub Actions runner security plan."""
import argparse,json,re,sys
from pathlib import Path
def validate(text):
    errors=[]
    for label,p in [("runner",r"(?i)\b(?:runner|ejecutor)\b"),("scope",r"(?i)\b(?:scope|alcance|organization|repository|group)\b"),("labels",r"(?i)\b(?:label|etiqueta)\b"),("isolation",r"(?i)\b(?:isolation|aislamiento|security|seguridad)\b"),("rollback",r"(?i)\b(?:rollback|revert|revers)\b")]:
        if not re.search(p,text): errors.append(f"Missing {label} information.")
    if len(text.split())<70: errors.append("Runner plan must contain at least 70 words.")
