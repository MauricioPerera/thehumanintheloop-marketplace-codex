#!/usr/bin/env python3
"""Validate a GitHub Actions permissions audit."""
import argparse,json,re,sys
from pathlib import Path
def validate(text):
    errors=[]
    for label,p in [("workflow",r"(?i)\b(?:workflow|workflows?)\b"),("permissions",r"(?i)\b(?:permissions?|permisos?|GITHUB_TOKEN)\b"),("scope",r"(?i)\b(?:scope|alcance|write|read)\b"),("third party",r"(?i)\b(?:third.party|terceros?|action)\b"),("rollback",r"(?i)\b(?:rollback|revert|revers)\b")]:
        if not re.search(p,text): errors.append(f"Missing {label} information.")
    if len(text.split())<70: errors.append("Action permissions audit must contain at least 70 words.")
