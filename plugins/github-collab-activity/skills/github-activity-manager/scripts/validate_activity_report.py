#!/usr/bin/env python3
"""Validate a GitHub activity report."""
import argparse,json,re,sys
from pathlib import Path
def validate(text):
    errors=[]
    for label,p in [("scope",r"(?i)\b(?:scope|alcance|repository|repositorio)\b"),("time",r"(?i)\b(?:time window|ventana|date|fecha|days|d[ií]as)\b"),("activity",r"(?i)\b(?:activity|actividad|commit|issue|pull request|review)\b"),("evidence",r"(?i)\b(?:evidence|evidencia|https?://)\b"),("limitations",r"(?i)\b(?:limitation|limitaci[oó]n|bot|pagination|paginaci[oó]n)\b")]:
        if not re.search(p,text):errors.append(f"Missing {label} information.")
    if len(text.split())<70:errors.append("Activity report must contain at least 70 words.")
