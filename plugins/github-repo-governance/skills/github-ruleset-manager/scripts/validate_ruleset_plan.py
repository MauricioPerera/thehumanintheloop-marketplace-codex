#!/usr/bin/env python3
"""Validate a GitHub ruleset change plan."""
import argparse,json,re,sys
from pathlib import Path
def validate(text):
    errors=[]
    for label,p in [("ruleset",r"(?i)\b(?:ruleset|regla|rule)\b"),("enforcement",r"(?i)\b(?:enforcement|enforced|cumplimiento)\b"),("bypass",r"(?i)\b(?:bypass|excepci[oó]n|exception)\b"),("checks",r"(?i)\b(?:check|status|validaci[oó]n)\b"),("rollback",r"(?i)\b(?:rollback|revert|revers)\b")]:
        if not re.search(p,text): errors.append(f"Missing {label} information.")
    if len(text.split())<70: errors.append("Ruleset plan must contain at least 70 words.")
