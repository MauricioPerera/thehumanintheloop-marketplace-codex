...
- `ephemeral-agent-delegator` (Ephemeral Agent Delegator): nuevo plugin (categoria Developer Tools). Delega tareas de implementacion a agentes efimeros por terminal, con dos backends: Ollama (`ollama launch claude`) y `pool exec` (poolside.ai). Basado en la doctrina de lanzamiento headless ya establecida para ambos CLIs (flags obligatorias, reglas anti-cuelgue, fallos transitorios y de cuota conocidos), generalizada y sin dependencias externas. El script `build_delegation_command.py` arma el comando exacto por backend pero **nunca lo ejecuta ni lo backgroundea el mismo** -- el agente orquestador debe correrlo con su propia tool de terminal en background, para no perder la notificacion de fin. Exige verificacion independiente del resultado en vez de confiar en el autoreporte del agente efimero. Probado: `probe`/`build` para ambos backends, modo `--goal` (Ollama), `--mcp-config-file` valido e invalido, y el caso de `--prompt-file` faltante -- encontro y corrigio un bug real (el modo `--goal` no deberia exigir `--prompt-file`, el primer intento fallaba).
